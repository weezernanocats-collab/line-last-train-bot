...
**各エージェントの発言は、Taskツールの結果を受け取った後、必ず以下のフォーマットでユーザーに表示すること。**
...
### 並列実行時の表示順序
第二幕・第三幕・第五幕では3人が並列で発言するが、表示は太郎→次郎→三郎の順で統一する。

### 表示の必須ルール
- Taskツールの結果は省略せず、全文をユーザーに見せる
- agentIdなどのメタ情報は除去して表示
- 各発言の間には空行を入れて読みやすくする

## 実行フロー（6幕構成）

Taskツールを使い、以下の順で実行する。
**各ステップの結果は上記フォーマットに従い、必ずユーザーに表示すること。**
